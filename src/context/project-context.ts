...
class ProjectContextManager {
  ...
  async setProjectRoot(absolutePath: string): Promise<void> {
    ...
    if (!absolutePath.startsWith('/') && !absolutePath.match(/^[A-Z]:\\/)) {
      throw new Error('Project root must be an absolute path (starting with / on Unix or drive letter on Windows)');
    }
    ...
    // Validate it's a valid project (has package.json or vitest.config)
    const hasPackageJson = await fileExists(join(absolutePath, 'package.json'));
    const hasVitestConfig = await fileExists(join(absolutePath, 'vitest.config.ts')) ||
    ...
    if (!hasPackageJson && !hasVitestConfig) {
      throw new Error(`Directory does not appear to be a valid project (no package.json or vitest.config found): ${absolutePath}`);
    }
    
    // Prevent running on the MCP package itself
    if (hasPackageJson) {
      try {
        ...
        if (packageData.name === '@djankies/vitest-mcp') {
          throw new Error('Cannot set project root to the Vitest MCP package itself. This tool is meant to test other projects, not itself.');
        }
      } catch (error) {
        // If it's our safety error, re-throw it
        if (error instanceof Error && error.message.includes('Vitest MCP package')) {
          throw error;
        }
        // Otherwise ignore read/parse errors and continue
      }
    }

    // Store the validated project root
